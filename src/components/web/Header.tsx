    ...
    return (
        <header className={`z-10 px-5 sm:px-10 md:px-16 lg:px-24  h-16 fixed w-screen transition duration-200 flex items-center top-0 left-0 justify-between ${scroll ? 'text-gray-700 bg-white shadow-lg' : 'text-white bg-transparent'}`}>
            ...
            <ul className={`w-full lg:flex lg:items-center lg:gap-5 ${isOpen ? 'gap-3 shadow-2xl flex flex-col fixed top-0 left-0 w-screen bg-white p-10 text-gray-600' : 'hidden'}`}>
                <ScrollLink to='about' duration={2000} smooth={true} className={`lg:ml-auto px-3 lg:px-2 xl:px-4 py-1.5 rounded-2xl cursor-pointer ${scroll && 'hover:text-blue-600'}`}>{t('about')}</ScrollLink>
                <ScrollLink to='services' duration={2000} smooth={true} className={`px-3 lg:px-2 xl:px-4 py-1.5 rounded-2xl cursor-pointer  ${scroll && 'hover:text-blue-600'}`}>{t('services')}</ScrollLink>
                ...
                <div className={`${isOpen ? 'mt-5' : ''} flex gap-5 items-center lg:gap-8  lg:ml-auto`}>
                    <ScrollLink to='contact' duration={2000} smooth={true} className={`hover:bg-gradient-to-b from-blue-700 via-blue-500 to-cyan-400 hover:text-white border px-5 lg:px-5 py-1.5 cursor-pointer flex items-center justify-center rounded-3xl ${scroll ? 'bg-blue-600 text-white' : 'bg-white text-blue-600'}`}>{t('waitlist')}</ScrollLink>
